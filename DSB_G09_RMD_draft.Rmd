...
author: "A. Stadelmann, S. Ahn, C. Ludwig, J. Legeren Alvarez, O. Ng"
output:
  html_document:
...
In the telecom industry, customer churn represents the number of customers that have stopped using the company’s services. This metric is an important indication for telecom companies as customer acquisition costs remain extremely high in the industry. Since the cost to acquire new customers is significantly higher than the costs to retain existing customers, high customer churn often means that telecom companies have higher costs to retain their user base. A telecom company’s ability to minimize churn is indicative of that company’s financial and operating performance. 

Our telecom company has gathered demographics data on over 7,000 customers and whether or not they have churned. Our goal is to determine which demographic make-up results in the greatest probability of churning.  
...
The "high level" process template is split in 3 parts, 

1. *Part 1*: We use some of the survey questions to find **key customer descriptors** ("factors") using *dimensionality reduction* techniques described in the [Dimensionality Reduction](http://inseaddataanalytics.github.io/INSEADAnalytics/CourseSessions/Sessions23/FactorAnalysisReading.html) reading of Sessions 7-8.
